...
### Remote Machines
Install a Linux virtual machine (or use an already existing one) for thiss exercise.If you are not familiar with virtual machines check out [this](https://hibbard.eu/install-ubuntu-virtual-box/) tutorial for installing one.
...
4. Start a webserver in your VM by executing **python3 -m http.server 8888**.Access the VM webserver by navigating to **http://localhost:9999** in your machine.
...
```bash
ssh -L 9999:example.org:80 -N -T root@romete_host
```
